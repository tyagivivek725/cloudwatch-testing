...
VERSION = "report_ppt v4.9 (get-metric-data + graceful no-data)"
...
def fetch_series(namespace, metric, dims, period, stat):
    ...
    try:
        ...
        if results and results[0].get("Timestamps"):
            pairs = sorted(zip(results[0]["Timestamps"], results[0].get("Values", [])), key=lambda t: t[0])
...
def plot_chart(xs, ys, ylabel, threshold=None, out_png=None, note=None):
    ...
    if xs and ys:
        ...
        ax.plot([], [])
        note = note or "No datapoints in selected window"
        ax.text(0.5, 0.5, note, transform=ax.transAxes, ha="center", va="center", fontsize=14)
    ...
    fig.subplots_adjust(left=0.08, right=0.995, top=0.98, bottom=0.22)
...
# Slide-1 date updater (looks for [DATE] or any date-like text)
MONTHS = ("January","February","March","April","May","June","July","August","September","October","November","December")
DATE_RX = re.compile(
    r"\b(\d{1,2})(st|nd|rd|th)[-\s](January|February|March|April|May|June|July|August|September|October|November|December)[-\s](\d{4})\b",
    re.IGNORECASE
)
def ordinal(n): return "th" if 10 <= n % 100 <= 20 else {1:"st",2:"nd",3:"rd"}.get(n % 10, "th")
def today_str():
    now = datetime.now().astimezone()
    return f"{now.day:02d}{ordinal(now.day)}-{MONTHS[now.month-1]}-{now.year}"
def update_first_slide_date(prs):
    if len(prs.slides) == 0: return
    s = prs.slides[0]; date_text = today_str()
    for shape in s.shapes:
        if not getattr(shape, "has_text_frame", False): continue
        try: txt = shape.text_frame.text
        except Exception: continue
        if "[DATE]" in txt:
            shape.text_frame.clear(); shape.text_frame.paragraphs[0].text = txt.replace("[DATE]", date_text); return
        if DATE_RX.search(txt):
            shape.text_frame.clear(); shape.text_frame.paragraphs[0].text = DATE_RX.sub(date_text, txt); return
...
def build_ppt(alarms, out_path):
    ...
    margin_x = Inches(0.4)           # tasteful side margins
    title_to_chart_top = Inches(1.45)
    details_height = Inches(2.63)      # taller for table
    bottom_margin = Inches(0.3)
